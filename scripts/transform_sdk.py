...
def generate_partner_apply_patches_sh(common_patch_dir: Path, output_path: Path,
    ...
    new_tar = (
        '    tar -zxf $COMMON_PATCH_DIR/$base_pkg -C $COMMON_PATCH_DIR/tmp/ '
        '"./vendor" "./ohos5/device" "./ohos5/vendor/hisilicon/$PRODUCT"'
        ...
        '    cp -Rf $COMMON_PATCH_DIR/tmp/ohos5/vendor/hisilicon/$PRODUCT/ $OHOS_PATH/vendor/hisilicon/'
    ...
    # 在文件顶部加注释和 PRODUCT 参数
    header = (
        '# Partner SDK 版本 (generated by transform_sdk.py)\n'
        '# 变更：移除 custom-sdk-vendor-patch，device/ 补丁已预集成到 SDK 包，\n'
        '#       vendor/hisilicon/$PRODUCT BUILD.gn 已预集成到 SDK 包（prebuilt 版本）\n'
        '# 用法: bash apply_patches_sdk_partner.sh [product]\n'
        '#   product: mp_hi3781v730（默认）或 mp_hi3781v735\n'
        f'PRODUCT="${{1:-{product}}}"\n'
    )
